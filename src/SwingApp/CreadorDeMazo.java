...
public class CreadorDeMazo extends JFrame {
	 ...
	 * Runnable() { public void run() { try { CreadorDeMazo frame = new
	 * CreadorDeMazo(); frame.setVisible(true); } catch (Exception e) {
	 * e.printStackTrace(); } } }); }
	 * 
	 * /** Create the frame.
	 */
	...
	public CreadorDeMazo() throws FileNotFoundException {
		...
		cantidadCoronas.setModel(new DefaultComboBoxModel<Integer>(new Integer[] { 0, 1, 2, 3 }));
	...
	protected void onModeloSeleccionado() {
		...
		Ficha f = new Ficha((String) tiposFicha.getSelectedItem(), cantidadCoronas.getSelectedIndex(), 0, 0, idFicha,
				null);
	...
	protected void onAgregarFichaClick() {
		...
		if (fichasActuales[0] == null) {
			fichasActuales[0] = new Ficha(tipo, (int) cantidadCoronas.getSelectedItem(), 0, 0, idFicha + 2,
					cartaActual);
		} else {
			fichasActuales[1] = new Ficha(tipo, (int) cantidadCoronas.getSelectedItem(), 0, 0, idFicha + 2,
					cartaActual);
			// cartaActual = new
			// Carta(fichasActuales[0].getId()/2,fichasActuales[0].getTipo(),fichasActuales[0].getCantCoronas(),fichasActuales[1].getTipo(),fichasActuales[1].getCantCoronas());
	...
	protected void visualizarMazo() {
		...
		for (Carta carta : cartas) {
			...
			pF.setBounds(i % 16 * PanelFicha.getTamFicha(), (i / 16) * PanelFicha.getTamFicha(),
					PanelFicha.getTamFicha(), PanelFicha.getTamFicha());
